...
int framerate = 60;
int numFaces = 10;
boolean doColorChange = true;
boolean doSpeedChange = false;
...
void setup() {
  ...
  for (int i=0; i<ballX.length; i++) {
    ...
    ballD[i]=random(300, 301);

    //Oh and the colors. a color variable takes RGB values and stores them.
    colors[i]=color(random(100, 255), random(100, 255), random(1, 255));

    //And to randomize the speed
    ballXSpeed[i]=random(.5, 1);
    ballYSpeed[i]=random(.5, 1);
...
void draw() {
  ...
  for (int i=0; i<ballX.length; i++) {
    ...
    if (ballX[i]+(ballD[i]/2)>1200) {
      ...
      if (doSpeedChange) {
        ballXSpeed[i]=random(2, 10);
        ballYSpeed[i]=random(2, 10);
      }
    }
